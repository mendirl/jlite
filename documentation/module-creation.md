...
So, as this is the business of JHLite you probably want to create a `io.mendirl.jhlite.my_module.domain` package. And you can start with a simple test: 
...
```java
package io.mendirl.jhlite.my_package;
...
To be used by JHLite, the `JHipsterModuleResource` needs to be a Spring bean so, let's create a configuration in `io.mendirl.jhlite.my_module.infrastructure.primary`:
...
For this to work, we'll need to add a simple orchestration class in `io.mendirl.jhlite.my_module.application`:
